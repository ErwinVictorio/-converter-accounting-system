...
class VatInputController extends Controller
{
        ...
        $target->update([
            ...
            'purchase_local_vat_amount' => round(
                $this->vatAmountBalance($target, 'purchase_local')
                    + $vatAmounts['purchase_local'],
                2
            ),
            'services_vat_amount' => round(
                $this->vatAmountBalance($target, 'services') + $vatAmounts['services'],
                2
            ),
    ...
    public function update(Request $request, VatInput $vatInput)
    {
        ...
        foreach (self::PURCHASE_RAW_VAT_FIELDS as $baseField => $rawField) {
            $usesVatInputs[$baseField] = $request->exists($rawField);
            $vatAmounts[$baseField] = $usesVatInputs[$baseField]
                ? round((float) ($validated[$rawField] ?? 0), 2)
                : $this->vatAmountFromBase($validated[$baseField] ?? 0);
            $amounts[$baseField] = $usesVatInputs[$baseField]
                ? $this->taxableBaseTransferFromVat($vatInput, $baseField, $vatAmounts[$baseField])
                : round((float) ($validated[$baseField] ?? 0), 2);
        }

        foreach (self::PURCHASE_RAW_VAT_FIELDS as $baseField => $rawField) {
            if ($usesVatInputs[$baseField]
                && $this->moneyToCents($vatAmounts[$baseField]) > $this->moneyToCents($this->vatAmountBalance($vatInput, $baseField))) {
                return back()
                    ->withErrors([$rawField => 'VAT amount cannot be greater than the available broker VAT amount.'])
                    ->withInput();
            }
        }
        ...
        foreach ($amounts as $field => $amount) {
            if ($amount > (float) $vatInput->{$field}) {
                return back()
                    ->withErrors([
                        $this->adjustmentRequestField($field, $usesVatInputs) => 'Amount cannot be greater than the original broker amount.',
                    ])
                    ->withInput();
            }
        }
        ...
        DB::transaction(function () use ($vatInput, $validated, $amounts, $newTotal, $vatAmounts, $usesVatInputs) {
            ...
            foreach (self::PURCHASE_RAW_VAT_FIELDS as $baseField => $rawField) {
                if ($usesVatInputs[$baseField]
                    && $this->moneyToCents($vatAmounts[$baseField]) > $this->moneyToCents($this->vatAmountBalance($sourceRecord, $baseField))) {
                    throw ValidationException::withMessages([
                        $rawField => 'The broker VAT balance changed. Review the available amounts and try again.',
                    ]);
                }

                if ($usesVatInputs[$baseField]) {
                    $amounts[$baseField] = $this->taxableBaseTransferFromVat(
                        $sourceRecord,
                        $baseField,
                        $vatAmounts[$baseField]
                    );
                }
            }
            ...
            foreach ($amounts as $field => $amount) {
                if ($amount > (float) $sourceRecord->{$field}) {
                    throw ValidationException::withMessages([
                        $this->adjustmentRequestField($field, $usesVatInputs) => 'The broker balance changed. Review the available amounts and try again.',
                    ]);
                }
            }
            ...
            if ($targetRecord) {
                ...
                $targetRecord = VatInput::create([
                    ...
                    'purchase_local_vat_amount' => $vatAmounts['purchase_local'],
                    'services' => $amounts['services'],
                    'services_vat_amount' => $vatAmounts['services'],
                    'capital_goods' => 0,
                    ...
                    'others_vat_amount' => $vatAmounts['others'],
                    'total' => $newTotal,
            ...
            $sourceRecord->update([
                ...
                'services_vat_amount' => round(
                    $this->vatAmountBalance($sourceRecord, 'services') - $vatAmounts['services'],
                    2
                ),
                'others_vat_amount' => round(
        ...
        try {
            DB::transaction(function () use ($vatInput, $validator) {
                ...
                foreach ($histories->groupBy('source_vat_input_id') as $sourceId => $sourceHistories) {
                    ...
                    foreach (self::PURCHASE_RAW_VAT_FIELDS as $field => $rawField) {
                        $rawCents = $sourceHistories->sum(fn (PurchaseAdjustment $history) => $this->moneyToCents(
                            $history->{$rawField} ?? $this->vatAmountFromBase($history->{$field})
                        ));
                    ...
                    $restored['purchase_imported_vat_amount'] = $this->centsToMoney(
                        $this->moneyToCents($this->vatAmountBalance($source, 'purchase_imported'))
                            + $sourceHistories->sum(fn (PurchaseAdjustment $history) => $this->moneyToCents(
                                $this->vatAmountFromBase($history->purchase_imported)
                            ))
                    );
    ...
    private function adjustmentCoverage(VatInput $target, $histories): array
    {
        ...
        foreach (self::PURCHASE_RAW_VAT_FIELDS as $baseField => $rawField) {
            if ($target->{$rawField} === null || $validHistories->isEmpty()) {
                continue;
            }

            $trackedVatCents = $validHistories->sum(fn (PurchaseAdjustment $history) => $this->moneyToCents(
                $history->{$rawField} ?? $this->vatAmountFromBase($history->{$baseField})
            ));
            $targetVatCents = $this->moneyToCents($target->{$rawField});

            if ($trackedVatCents > $targetVatCents) {
                $invalid = true;
            }

            $complete = $complete && $trackedVatCents === $targetVatCents;
        }
    ...
    private function storeLegacyAdjustmentLinks(VatInput $target, array $unresolvedCents, $allocations): void
    {
        ...
        foreach ($allocations as $allocation) {
            ...
            foreach (self::PURCHASE_RAW_VAT_FIELDS as $baseField => $rawField) {
                $row[$rawField] = $this->centsToMoney(
                    $this->moneyToCents($this->vatAmountFromBase($row[$baseField]))
                );
            }
    ...
    private function taxableBaseTransferFromVat(VatInput $record, string $field, mixed $vatAmount): float
    {
        if ($this->moneyToCents($vatAmount) === $this->moneyToCents($this->vatAmountBalance($record, $field))) {
            return round((float) $record->{$field}, 2);
        }

        return $this->taxableBaseFromVat($vatAmount);
    }
    ...
    /**
     * @param  array<string, bool>  $usesVatInputs
     */
    private function adjustmentRequestField(string $baseField, array $usesVatInputs): string
    {
        return ($usesVatInputs[$baseField] ?? false)
            ? self::PURCHASE_RAW_VAT_FIELDS[$baseField]
            : $baseField;
    }
