...
`Annual` uses `Start Date` and `End Date`, and the covered period must be one full taxable year. Annual Expanded WTAX requires a full taxable year. Select January 1 as Start Date and December 31 of the same year as End Date. Annual upload validates that every workbook row date falls inside that year and stores each row under its own row month. Annual rows are marked with `report_type = annual`, and the Generate DAT screen generates a 1604E Schedule 3 DAT for valid annual rows in that taxable year.
...
- For `Annual`, the row `Date` must be inside the selected taxable year, January 1 to December 31.
...
- For `Annual`, every row's `Reporting_Month` must be inside the selected taxable year, January 1 to December 31.
...
Expanded WTAX DAT generation is per withholding agent/company TIN and branch.
...
- start date and end date for `Annual`, which must be January 1 and December 31 of the same year

Annual Expanded WTAX requires a full taxable year. Select January 1 as Start Date and December 31 of the same year as End Date. A partial covered period is refused with:
...
Do not use the annual 1604E filename shape for quarterly 1601EQ validator upload:
...
Annual generation uses the 1604E Schedule 3 filename shape:

```text
{TIN}{BRANCH_CODE}{MMDDYYYY}1604E.dat
```

For Fortress Steel taxable year `2026`:

```text
0087919760000123120261604E.dat
```

The selected taxable year decides which rows are included: January 1 to December 31 of that year. The DAT file date is that same taxable year end, `12/31/YYYY`, because the AVS 1604E validator compares the file date with the Tax Year entered on the validation screen. That is also why a partial covered period is refused instead of being widened to `12/31` — the file would claim a full year it does not hold.
...
- months covered by the selected taxable year for `Annual`
...
Expanded WTAX has two separate DAT body formats:

- Quarterly: `1601EQ/QAP`
- Annual: `1604E Schedule 3`
...
Field counts are fixed: header 4, detail 16, control 6.

Annual detail rows repeat the withholding agent TIN, branch, and period end date before the payee fields. This is different from Quarterly 1601EQ, where detail rows start with the sequence number after `D1,1601EQ`.

Annual uses the selected taxable year, which the app has already validated as January 1 to December 31 of one year, and writes that year's `12/31/YYYY` as the DAT period end date.
...
```text
C3,1604E,008791976,0000,12/31/2026,36847.16
```
...
- reporting month for Quarterly, or selected annual period end for Annual
...
The annual generator test includes a byte-for-byte check against `Docs/Expanded/0087919760000123120251604E.dat`.
